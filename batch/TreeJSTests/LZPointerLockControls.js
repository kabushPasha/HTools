...
function onMouseMove(event) {
    ...
    // Yaw rotation around upAxis
    yawQuat.setFromAxisAngle(this.upAxis, -event.movementX * _MOUSE_SENSITIVITY * this.pointerSpeed);

    // Pitch rotation around right axis
    const right = new Vector3();
    camera.getWorldDirection(right);       // forward
    right.cross(this.upAxis).normalize();  // right vector perpendicular to up and forward
    pitchQuat.setFromAxisAngle(right, -event.movementY * _MOUSE_SENSITIVITY * this.pointerSpeed);
