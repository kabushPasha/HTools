...
typedef boost::tuple<UT_DeepString, UT_DeepString, UT_FileStat> iconInfo;
...
main(int argc, char *argv[])
{
    ...
    if (argc == 2)
    {
	strncpy(icondir, argv[1], BUFSIZ);
        strncpy(destdir, argv[1], BUFSIZ);
    }
    else if (argc == 3)
    {
        strncpy(icondir, argv[1], BUFSIZ);
        strncpy(destdir, argv[2], BUFSIZ);
    }
    else
	strcpy(icondir, ".");

    sprintf(iconindex, "%s/SVGIcons.index", destdir);
    ...
    for (i = 0; i < iconlist.entries(); ++i)
    {
	const char *name = iconlist(i).get<0>();
	const char *section_name = iconlist(i).get<1>();
	struct stat stats;
